...
private fun mapMlsException(context: JsErrorContext?, fallbackMessage: String?): MlsException = when (context?.type) {
    "ConversationAlreadyExists" -> MlsException.ConversationAlreadyExists(
        (context.conversationId ?: emptyArray()).map {
            it.toByte()
        }.toByteArray()
    )
...
private fun newJsConversationId(bytes: ByteArray): JsConversationId = constructJsBytesType(
    "ConversationId",
    bytes
).unsafeCast<JsConversationId>()
...
private fun newJsExternalSenderKey(bytes: ByteArray): JsExternalSenderKey = constructJsBytesType(
    "ExternalSenderKey",
    bytes
).unsafeCast<JsExternalSenderKey>()
...
private fun newJsMlsTransportData(bytes: ByteArray): JsMlsTransportData = constructJsBytesType(
    "MlsTransportData",
    bytes
).unsafeCast<JsMlsTransportData>()
...
private fun encryptionTypeFromJs(value: Int): MlsGroupInfoEncryptionType = MlsGroupInfoEncryptionType.entries.first {
    it.value.toInt() ==
        value
}
...
private fun e2eiConversationStateFromJs(value: Int): E2eiConversationState = E2eiConversationState.entries.first {
...
private fun mapTransportResponse(response: dynamic): MlsTransportResponse = when {
    ...
    else -> CoreCryptoException.Other("Unknown MLS transport response: $response").let { throw it }
}
...
public actual open class DatabaseKey : Disposable, DatabaseKeyInterface {
    ...
    actual override fun equals(other: Any?): Boolean =
        other is DatabaseKey && (bytesHandle?.bytes() ?: byteArrayOf()).contentEquals(other.bytesHandle?.bytes() ?: byteArrayOf())
...
public actual open class E2eiEnrollment : Disposable, E2eiEnrollmentInterface {
    ...
    public actual override suspend fun certificateRequest(previousNonce: String): ByteArray = webCall {
        requireJs().certificateRequest(previousNonce)
    }.toByteArray()

    public actual override suspend fun checkOrderRequest(orderUrl: String, previousNonce: String): ByteArray = webCall {
        requireJs().checkOrderRequest(orderUrl, previousNonce)
    }.toByteArray()

    public actual override suspend fun checkOrderResponse(order: ByteArray): String = webCall {
        requireJs().checkOrderResponse(order.toUint8Array())
    }

    public actual override suspend fun createDpopToken(expirySecs: UInt, backendNonce: String): String = webCall {
        requireJs().createDpopToken(expirySecs.toInt(), backendNonce)
    }.toByteArray().decodeToString()

    public actual override suspend fun directoryResponse(directory: ByteArray): AcmeDirectory = mapAcmeDirectory(
        webCall {
            requireJs().directoryResponse(directory.toUint8Array())
        }
    )

    public actual override suspend fun finalizeRequest(previousNonce: String): ByteArray = webCall {
        requireJs().finalizeRequest(previousNonce)
    }.toByteArray()

    public actual override suspend fun finalizeResponse(finalize: ByteArray): String = webCall {
        requireJs().finalizeResponse(finalize.toUint8Array())
    }

    public actual override suspend fun newAccountRequest(previousNonce: String): ByteArray = webCall {
        requireJs().newAccountRequest(previousNonce)
    }.toByteArray()

    public actual override suspend fun newAccountResponse(account: ByteArray) {
        webCall { requireJs().newAccountResponse(account.toUint8Array()) }
    }

    public actual override suspend fun newAuthzRequest(url: String, previousNonce: String): ByteArray = webCall {
        requireJs().newAuthzRequest(url, previousNonce)
    }.toByteArray()

    public actual override suspend fun newAuthzResponse(authz: ByteArray): NewAcmeAuthz = mapNewAcmeAuthz(
        webCall {
            requireJs().newAuthzResponse(authz.toUint8Array())
        }
    )

    public actual override suspend fun newDpopChallengeRequest(accessToken: String, previousNonce: String): ByteArray = webCall {
        requireJs().newDpopChallengeRequest(accessToken, previousNonce)
    }.toByteArray()

    public actual override suspend fun newDpopChallengeResponse(challenge: ByteArray) {
        webCall { requireJs().newDpopChallengeResponse(challenge.toUint8Array()) }
    }

    public actual override suspend fun newOidcChallengeRequest(idToken: String, previousNonce: String): ByteArray = webCall {
        requireJs().newOidcChallengeRequest(idToken, previousNonce)
    }.toByteArray()

    public actual override suspend fun newOidcChallengeResponse(challenge: ByteArray) {
        webCall { requireJs().newOidcChallengeResponse(challenge.toUint8Array()) }
    }

    public actual override suspend fun newOrderRequest(previousNonce: String): ByteArray = webCall {
        requireJs().newOrderRequest(previousNonce)
    }.toByteArray()

    public actual override suspend fun newOrderResponse(order: ByteArray): NewAcmeOrder = mapNewAcmeOrder(
        webCall {
            requireJs().newOrderResponse(order.toUint8Array())
        }
    )
...
public actual open class CoreCrypto : Disposable, CoreCryptoInterface {
    ...
    public actual override fun proteusFingerprintPrekeybundle(prekey: ByteArray): String = jsCoreCryptoProteusFingerprintPrekeybundle(
        prekey.toUint8Array()
    )

    public actual override suspend fun proteusFingerprintLocal(sessionId: String): String = webCall {
        requireJs().proteusFingerprintLocal(sessionId)
    }

    public actual override suspend fun proteusFingerprintRemote(sessionId: String): String = webCall {
        requireJs().proteusFingerprintRemote(sessionId)
    }

    public actual override fun proteusLastResortPrekeyId(): UShort = jsCoreCryptoProteusLastResortPrekeyId().toUShort()

    public actual override suspend fun proteusSessionExists(sessionId: String): Boolean = webCall {
        requireJs().proteusSessionExists(sessionId)
    }

    public actual override suspend fun provideTransport(callbacks: MlsTransport) {
        webCall { requireJs().provideTransport(mlsTransportToJs(callbacks)) }
    }

    public actual override suspend fun randomBytes(len: UInt): ByteArray = webCall { requireJs().randomBytes(len.toInt()) }.toByteArray()

    public actual override suspend fun registerEpochObserver(epochObserver: EpochObserver) {
        webCall { requireJs().registerEpochObserver(epochObserverToJs(epochObserver)) }
    }

    public actual override suspend fun registerHistoryObserver(historyObserver: HistoryObserver) {
        webCall { requireJs().registerHistoryObserver(historyObserverToJs(historyObserver)) }
    }

    public actual override suspend fun reseedRng(seed: ByteArray) {
        webCall { requireJs().reseedRng(seed.toUint8Array()) }
    }
...
public actual open class CoreCryptoContext : Disposable, CoreCryptoContextInterface {
    ...
    public actual override suspend fun clientValidKeypackagesCount(ciphersuite: Ciphersuite, credentialType: CredentialType): ULong = dynamicToULong(
        webCall {
            requireJs().clientValidKeypackagesCount(ciphersuiteToJs(ciphersuite), credentialTypeToJs(credentialType))
        }
    )

    public actual override suspend fun commitPendingProposals(conversationId: ConversationId) {
        webCall { requireJs().commitPendingProposals(conversationId.requireJs()) }
    }

    public actual override suspend fun conversationCiphersuite(conversationId: ConversationId): Ciphersuite = ciphersuiteFromJs(
        webCall {
            requireJs().conversationCiphersuite(conversationId.requireJs())
        }.toInt()
    )

    public actual override suspend fun conversationEpoch(conversationId: ConversationId): ULong = dynamicToULong(
        webCall {
            requireJs().conversationEpoch(conversationId.requireJs())
        }
    )
    ...
    public actual override suspend fun createConversation(conversationId: ConversationId, creatorCredentialType: CredentialType, config: ConversationConfiguration) {
        webCall {
            requireJs().createConversation(
                conversationId.requireJs(),
                credentialTypeToJs(creatorCredentialType),
                conversationConfigurationToJs(config)
            )
        }
    }

    public actual override suspend fun decryptMessage(conversationId: ConversationId, payload: ByteArray): DecryptedMessage = mapDecryptedMessage(
        webCall {
            requireJs().decryptMessage(conversationId.requireJs(), payload.toUint8Array())
        }
    )

    public actual override suspend fun deleteStaleKeyPackages(ciphersuite: Ciphersuite) {
        webCall { requireJs().deleteStaleKeyPackages(ciphersuiteToJs(ciphersuite)) }
    }

    public actual override suspend fun disableHistorySharing(conversationId: ConversationId) {
        webCall { requireJs().disableHistorySharing(conversationId.requireJs()) }
    }
    ...
    public actual override suspend fun e2eiEnrollmentStash(enrollment: E2eiEnrollment): ByteArray = webCall {
        requireJs().e2eiEnrollmentStash(enrollment.js ?: throw CoreCryptoException.Other("E2eiEnrollment is not initialized"))
    }.toByteArray()

    public actual override suspend fun e2eiEnrollmentStashPop(handle: ByteArray): E2eiEnrollment = E2eiEnrollment(
        webCall {
            requireJs().e2eiEnrollmentStashPop(handle.toUint8Array())
        }
    )
    ...
    public actual override suspend fun e2eiMlsInitOnly(enrollment: E2eiEnrollment, certificateChain: String, nbKeyPackage: UInt?): List<String>? = webCall {
        requireJs().e2eiMlsInitOnly(
            enrollment.js ?: throw CoreCryptoException.Other("E2eiEnrollment is not initialized"),
            certificateChain,
            nbKeyPackage?.toInt()
        )
    }?.toList()

    public actual override suspend fun e2eiNewActivationEnrollment(displayName: String, handle: String, team: String?, expirySec: UInt, ciphersuite: Ciphersuite): E2eiEnrollment = E2eiEnrollment(
        webCall {
            requireJs().e2eiNewActivationEnrollment(displayName, handle, expirySec.toInt(), ciphersuiteToJs(ciphersuite), team)
        }
    )

    public actual override suspend fun e2eiNewEnrollment(clientId: String, displayName: String, handle: String, team: String?, expirySec: UInt, ciphersuite: Ciphersuite): E2eiEnrollment = E2eiEnrollment(
        webCall {
            requireJs().e2eiNewEnrollment(clientId, displayName, handle, expirySec.toInt(), ciphersuiteToJs(ciphersuite), team)
        }
    )

    public actual override suspend fun e2eiNewRotateEnrollment(displayName: String?, handle: String?, team: String?, expirySec: UInt, ciphersuite: Ciphersuite): E2eiEnrollment = E2eiEnrollment(
        webCall {
            requireJs().e2eiNewRotateEnrollment(expirySec.toInt(), ciphersuiteToJs(ciphersuite), displayName, handle, team)
        }
    )

    public actual override suspend fun e2eiRegisterAcmeCa(trustAnchorPem: String) {
        webCall { requireJs().e2eiRegisterAcmeCA(trustAnchorPem) }
    }

    public actual override suspend fun e2eiRegisterCrl(crlDp: String, crlDer: ByteArray): CrlRegistration = mapCrlRegistration(
        webCall {
            requireJs().e2eiRegisterCRL(crlDp, crlDer.toUint8Array())
        }
    )

    public actual override suspend fun e2eiRegisterIntermediateCa(certPem: String): List<String>? = webCall {
        requireJs().e2eiRegisterIntermediateCA(certPem)
    }?.toList()

    public actual override suspend fun e2eiRotate(conversationId: ConversationId) {
        webCall { requireJs().e2eiRotate(conversationId.requireJs()) }
    }

    public actual override suspend fun enableHistorySharing(conversationId: ConversationId) {
        webCall { requireJs().enableHistorySharing(conversationId.requireJs()) }
    }

    public actual override suspend fun encryptMessage(conversationId: ConversationId, message: ByteArray): ByteArray = webCall {
        requireJs().encryptMessage(conversationId.requireJs(), message.toUint8Array())
    }.toByteArray()

    public actual override suspend fun exportSecretKey(conversationId: ConversationId, keyLength: UInt): SecretKey = SecretKey(
        webCall {
            requireJs().exportSecretKey(conversationId.requireJs(), keyLength.toInt())
        }
    )
    ...
    public actual override suspend fun getExternalSender(conversationId: ConversationId): ExternalSenderKey = ExternalSenderKey(
        webCall {
            requireJs().getExternalSender(conversationId.requireJs())
        }
    )
    ...
    public actual override suspend fun getUserIdentities(conversationId: ConversationId, userIds: List<String>): Map<String, List<WireIdentity>> = mapUserIdentities(
        webCall {
            requireJs().getUserIdentities(conversationId.requireJs(), userIds.toTypedArray())
        }
    )

    public actual override suspend fun joinByExternalCommit(groupInfo: GroupInfo, customConfiguration: CustomConfiguration, credentialType: CredentialType): WelcomeBundle = mapWelcomeBundle(
        webCall {
            requireJs().joinByExternalCommit(
                groupInfo.requireJs(),
                credentialTypeToJs(credentialType),
                customConfigurationToJs(customConfiguration)
            )
        }
    )

    public actual override suspend fun markConversationAsChildOf(childId: ConversationId, parentId: ConversationId) {
        webCall { requireJs().markConversationAsChildOf(childId.requireJs(), parentId.requireJs()) }
    }

    public actual override suspend fun mlsInit(clientId: ClientId, ciphersuites: List<Ciphersuite>, nbKeyPackage: UInt?) {
        webCall { requireJs().mlsInit(clientId.requireJs(), ciphersuites.map(::ciphersuiteToJs).toTypedArray(), nbKeyPackage?.toInt()) }
    }

    public actual override suspend fun processWelcomeMessage(welcomeMessage: Welcome, customConfiguration: CustomConfiguration): WelcomeBundle = mapWelcomeBundle(
        webCall {
            requireJs().processWelcomeMessage(welcomeMessage.requireJs(), customConfigurationToJs(customConfiguration))
        }
    )

    public actual override suspend fun proteusDecrypt(sessionId: String, ciphertext: ByteArray): ByteArray = webCall {
        requireJs().proteusDecrypt(sessionId, ciphertext.toUint8Array())
    }.toByteArray()
    ...
    public actual override suspend fun proteusEncrypt(sessionId: String, plaintext: ByteArray): ByteArray = webCall {
        requireJs().proteusEncrypt(sessionId, plaintext.toUint8Array())
    }.toByteArray()
    ...
    public actual override fun proteusFingerprintPrekeybundle(prekey: ByteArray): String = jsCoreCryptoContextProteusFingerprintPrekeybundle(
        prekey.toUint8Array()
    )
    ...
    public actual override suspend fun proteusInit() {
        webCall { requireJs().proteusInit() }
    }

    public actual override suspend fun proteusLastResortPrekey(): ByteArray = webCall {
        requireJs().proteusLastResortPrekey()
    }.toByteArray()

    public actual override suspend fun proteusNewPrekey(prekeyId: UShort): ByteArray = webCall {
        requireJs().proteusNewPrekey(prekeyId.toInt())
    }.toByteArray()

    public actual override suspend fun proteusNewPrekeyAuto(): ProteusAutoPrekeyBundle = mapProteusAutoPrekeyBundle(
        webCall {
            requireJs().proteusNewPrekeyAuto()
        }
    )

    public actual override suspend fun proteusReloadSessions() {
        webCall { requireJs().proteusReloadSessions() }
    }

    public actual override suspend fun proteusSessionDelete(sessionId: String) {
        webCall { requireJs().proteusSessionDelete(sessionId) }
    }
    ...
    public actual override suspend fun proteusSessionFromMessage(sessionId: String, envelope: ByteArray): ByteArray = webCall {
        requireJs().proteusSessionFromMessage(sessionId, envelope.toUint8Array())
    }.toByteArray()

    public actual override suspend fun proteusSessionFromPrekey(sessionId: String, prekey: ByteArray) {
        webCall { requireJs().proteusSessionFromPrekey(sessionId, prekey.toUint8Array()) }
    }

    public actual override suspend fun proteusSessionSave(sessionId: String) {
        webCall { requireJs().proteusSessionSave(sessionId) }
    }

    public actual override suspend fun randomBytes(len: UInt): ByteArray = webCall { requireJs().randomBytes(len.toInt()) }.toByteArray()

    public actual override suspend fun removeClientsFromConversation(conversationId: ConversationId, clients: List<ClientId>) {
        webCall { requireJs().removeClientsFromConversation(conversationId.requireJs(), clients.map { it.requireJs() }.toTypedArray()) }
    }

    public actual override suspend fun saveX509Credential(enrollment: E2eiEnrollment, certificateChain: String): List<String>? = webCall {
        requireJs().saveX509Credential(
            enrollment.js ?: throw CoreCryptoException.Other("E2eiEnrollment is not initialized"),
            certificateChain
        )
    }?.toList()

    public actual override suspend fun setData(data: ByteArray) {
        webCall { requireJs().setData(data.toUint8Array()) }
    }

    public actual override suspend fun updateKeyingMaterial(conversationId: ConversationId) {
        webCall { requireJs().updateKeyingMaterial(conversationId.requireJs()) }
    }

    public actual override suspend fun wipeConversation(conversationId: ConversationId) {
        webCall { requireJs().wipeConversation(conversationId.requireJs()) }
    }
...
public actual open class CoreCryptoLoggerImpl : Disposable, CoreCryptoLogger {
    ...
    public actual override fun log(level: CoreCryptoLogLevel, message: String, context: String?) {
...
public actual fun ciphersuiteFromU16(discriminant: UShort): Ciphersuite = ciphersuiteFromJs(
    JsCoreCryptoModule.ciphersuiteFromU16(discriminant.toShort()).toInt()
)
...
public actual suspend fun openDatabase(name: String, key: DatabaseKey): Database = Database(
    webCall {
        JsCoreCryptoModule.openDatabase(name, key.requireJs())
    }
)
