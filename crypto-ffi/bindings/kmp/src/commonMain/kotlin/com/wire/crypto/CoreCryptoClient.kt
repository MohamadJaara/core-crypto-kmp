 ...
 */
public suspend fun historyClient(historySecret: HistorySecret): CoreCryptoClient =
    coreCryptoHistoryClient(historySecret).lift()
...
public class CoreCryptoClient(private val cc: CoreCrypto) {
     ...
     */
    public suspend fun registerEpochObserver(scope: CoroutineScope, epochObserver: EpochObserver) {
     ...
     * This function should be called 0 or 1 times in the lifetime of CoreCrypto, regardless of the number of transactions.
     */
    public suspend fun registerHistoryObserver(scope: CoroutineScope, historyObserver: HistoryObserver) {
        // we want to wrap the observer here to provide async indirection, so that no matter what
        // the observer that makes its way to the Rust side of things doesn't end up blocking
